...
%============== load the SCPperiod_secfilter bank characteristics =====================
...
for ihc = 1 %:8,  ihc is the site - index of H and C
    ...
    for ifile=1:nbmats, ifile,
        ...
        %====== Useful evaluated parameters for general purposes
        for ip=1:Pfilter
            frqs_Hz{ip} = SUTs(ip).frqsFFT_Hz(idipinf(ip):idipsup(ip))';
...
%         end
    ...
    if FLAGsavesmall
        comsave = sprintf('save %s/resultssta26sensor%i',...
