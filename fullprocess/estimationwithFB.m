...
% directorydatafromIDC is used if FLAGsavesmall=1;
%==========================================================================
...
%======
% if generated by geneFB.m, use LOAD; if a .m program use RUN
if 1
...
%     directoryresults if FLAGsavesmall==1
...
% indexofSTA==2, ifile== 33, i.e. sta2_Y2015_D219.mat from sample index 2.4e6
% indexofSTA==8, ifile==63, i.e. sta8_Y2015_D280.mat from sample index 2.5e6
% indexofSTA==5,ifile==62
% indexofSTA==6,ifile==63
%??????????
%=====================
MSCthreshold = 0.98;
%=====================
for indexofSTA = [5,6]
    ...
    for ifile=1:nbmats, %ifile,%tic
        ...
        %== some manual checks ...
        if and(indexofSTA==2,ifile==33)
                signals_centered = signals_centered(1:2.4e6,:);
        end
        
        
        if and(indexofSTA==2,ifile==62)
            signals_centered = signals_centered([1:0.5e6 1e6:idScMin-1],:);
        end

        if and(indexofSTA==5,ifile==62)
            signals_centered = signals_centered(1:2.4e6,:);
        end

        if and(indexofSTA==6,ifile==63)
            signals_centered = signals_centered(1:2.14e6,:);
        end
        
        if and(indexofSTA==8,ifile==63)
