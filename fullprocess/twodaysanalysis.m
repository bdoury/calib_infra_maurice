...
% filtercharactfilename = 'filtercharacteristics/filtercharacteristics0.m';
...
figure(1)
clf
pos_ip = [0.1300    0.2616    0.7750    0.0934];
deltay = 1*pos_ip(4);
for ip=1:P
%     subplot(P,1,P-ip+1)
    subplot('position',pos_ip);
%     set(gca,'position',pos_ip);
    hpcolor=pcolor(alltimes_sec{ip}.SD/3600,allfrqsFFT_Hz{ip}(idipinf(ip):idipsup(ip)),10*log10(SUTs(ip).estimRsup.tabmod((idipinf(ip):idipsup(ip)),:)));
    caxis([-10 10])
    shading flat
    if ip>1
    set(gca,'xtick',[])
    end
    set(gca,'ytick',[allfrqsFFT_Hz{ip}([idipinf(ip)+1 idipsup(ip)-1])])
    set(gca,'yscale','log')
    set(gca,'box','on')
