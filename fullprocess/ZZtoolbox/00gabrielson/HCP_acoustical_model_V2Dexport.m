...
%
%================= USER INPUT BLOCK =======================================
element = 'I99V2_BFL';  %  For plot annotation
%                             (V2_BFL is 36m HCP at I99)
%                             (V2_BFH is 18m HCP at I99)
% HCP_aperture = 36;  HCP_order = 3;  %  36m HCP at I99
HCP_aperture = 18;  HCP_order = 2;  %  18m HCP at I99
%
arrival_elevation_degrees = [0, 30];  %  Elevation angle: horizontal = 0
...
L_pri = 5.75;  L_sec = 3.25;  L_ter = 2.07;  % lengths of pipes 18-m HCP
%
...
L_stub = 12.24 + 1.16;  a_stub = a_sec; % 36-m HCP
% % L_stub = 12.24 + 1.16;  a_stub = a_sec; % 18-m HCP
%
%  Cavity specs (microbarometer, primary, secondary, 
%                     and tertiary manifolds)
%
a_microbarometer = 70e-3;  %  radius and length to get correct MB2000
L_microbarometer = 39e-3;  %     volume (0.0006 m^3 from Alcoverro)
%
a_ter_cavity = 85e-3/2;  % 85 mm diameter
L_ter_cavity = 50e-3;    % 50 mm height
...
tempC = 5;  atmos_press = 88000;  %   88000 Pa is roughly "standard" 
%                                  atmospheric pressure at the altitude of
%                                  Conrad Observatory
%
%   Select frequencies for solution
freq_vector = (logspace(log10(0.001), log10(100), 501)).';
%        [0.001 to 100 Hz, log spacing]
%
plot_f_lo = 4e-3;  plot_f_hi = 20;  %  range of frequencies to display
%
%     'damping_correction' is an empirical correction factor used in 
%  Tmatrix_air.  Set to zero to disable that correction.
damping_correction = 1;  %  normally set to one
%
%     Set 'save_file' to true to save response as .mat file
save_file = false;
  save_file_name = 'insert a file name';  % only used if save_file = true
%
...
tempK = tempC + 273;  cc = 20.05*sqrt(tempK);
MW_air = 28.97;  Rgas = 8314.3;  rho = atmos_press*MW_air/(Rgas*tempK);
%
...
min_wavelength = cc/freq_vector(end);  max_dx = min_wavelength/10;
%
...
N_elev_angles = length(arrival_elevation_degrees);
p_total = zeros(length(freq_vector), N_elev_angles);
%
for kk = 1:N_elev_angles
    p_inside = PW_response(freq_vector, cc,...
        arrival_elevation_radians(kk), arrival_azimuth_radians, xx, yy);
    %   Combined acosutical and spatial response
    p_total(:, kk) = p_ratio.*p_inside;
end
%
%   Set up for plotting
%
...
title_textRS = ['   ', element, ':  Suppressors: '];
title_RS_0 = 'None';  title_RS_S = 'Sec ';  title_RS_P = 'Pri ';
title_RS_1 = '';
if RS_at_secondary_manifold
    title_RS_1 = title_RS_S;
end
if RS_at_primary_manifold
    title_RS_1 = [title_RS_1, title_RS_P];
end
if isempty(title_RS_1)
    title_text1 = [title_textRS, title_RS_0];
else
    title_text1 = [title_textRS, title_RS_1];
end
title_text2 = ['Elevation Angle [deg] = ',...
    num2str(arrival_elevation_degrees), ';  Stub [m] = ', num2str(L_stub)];
title_text = char(title_text1, title_text2);
%
%  Plot response prediction
%
line_color = ['k-'; 'b-'; 'r-'; 'g-'; 'k:'; 'b:'; 'r:'; 'g:'];
%
figure(1)  %  Magnitude
set(gcf, 'position', position)
for kk = 1:N_elev_angles
    hh = loglog(freq_vector, abs(p_total(:, kk)),...
        line_color(kk, :));
    set(hh, 'linewidth', 2)
    hold on
end
...
title(title_text, 'fontsize', 18, 'interpreter', 'none')
grid on
hold off
...
set(gcf, 'position', position)
for kk = 1:N_elev_angles
    hh = semilogx(freq_vector, (180/pi)*angle(p_total(:, kk)),...
        line_color(kk, :));
    set(hh, 'linewidth', 2)
    hold on
end
...
if save_file
    %
    %   Save configuration details as a data structure
    config.element = element;
    config.Npri  = Npri;   config.Nsec  = Nsec;   config.Nter  = Nter;
    config.L_pri = L_pri;  config.L_sec = L_sec;  config.L_ter = L_ter;
    config.a_pri = a_pri;  config.a_sec = a_sec;  config.a_ter = a_ter;
    config.L_stub = L_stub;
    if L_stub > 0
        config.a_stub = a_stub;
    end
    config.a_microbarometer = a_microbarometer; 
    config.L_microbarometer = L_microbarometer;
    config.a_pri_cavity     = a_pri_cavity;
    config.L_pri_cavity     = L_pri_cavity;
    config.a_sec_cavity     = a_sec_cavity;
    config.L_sec_cavity     = L_sec_cavity;
    config.a_ter_cavity     = a_ter_cavity;
    config.L_ter_cavity     = L_ter_cavity;
    config.tempC = tempC;  config.atmos_press = atmos_press;
    config.RS_at_secondary_manifold = RS_at_secondary_manifold; 
    config.RS_at_primary_manifold   = RS_at_primary_manifold;
    config.damping_correction = damping_correction;
    config.arrival_elevation_degrees = arrival_elevation_degrees;
    config.arrival_azimuth_degrees   = arrival_azimuth_degrees;
    if RS_at_secondary_manifold
        config.a_rss     = a_rss;
        config.L_rss     = L_rss;
    end
    ...
    %
    save(save_file_name, 'config', 'freq_vector', 'p_total', 'p_ratio')
end
