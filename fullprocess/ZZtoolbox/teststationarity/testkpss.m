function [h,pValue,stat,cValue,reg] = testkpss(y,varargin)
...
%   NAME        VALUE
%
%   'lags'      Scalar or vector of non-negative integers indicating the
...
%  
% See also PPTEST, ADFTEST, VRATIOTEST. 

% Copyright 1999-2009 The MathWorks, Inc.
% $Revision: 1.1.6.2 $ $Date: 2009/05/07 18:18:31 $
...
if T < 1
    error('econ:kpsstest:EffectiveSampleSizeLessThanOne',...
          'Effective sample size of the data is less than one.')    
end

if any(trend) && (T < 2)
	error('econ:kpsstest:OnePointTrend',...
          'At least two observations are needed to fit a trend.')
end
...
for i = 1:numTests
    ...
    if ~sameStat % Recompute the statistic only if it changes
        ...
        [testStat,testPValue] = getStat(i,sigLevels,T,testReg,CVTable,needPValue);
...
function OK = yCheck(y)
    ...
    if isempty(y)
        error('econ:kpsstest:DataUnspecified',...
              'Observed data is empty.')
    elseif ~isnumeric(y)
        error('econ:kpsstest:DataNonNumeric',...
              'Observed data must be numeric.')
    elseif ~isvector(y)
        error('econ:kpsstest:DataIsArray',...
              'Observed data must be a vector.')
    else
        OK = true;
    end
...
function OK = lagsCheck(lags)
    ...
    if ~isnumeric(lags)
        error('econ:kpsstest:LagsNonNumeric',...
              'Lags must be numeric.')
    elseif ~isvector(lags)
        error('econ:kpsstest:LagsIsArray',...
              'Lags must be a scalar or a vector.')
    elseif any(mod(lags,1) ~= 0) || any(lags < 0)
        error('econ:kpsstest:LagsOutOfRange',...
...
function OK = trendCheck(trend)
    ...
    if ~islogical(trend)
        error('econ:kpsstest:TrendNonBoolean',...
              'Trend flag must be Boolean.')
    elseif ~isvector(trend)
        error('econ:kpsstest:TrendIsArray',...
              'Trend flag must be a scalar or a vector.')
    else
        OK = true;
    end
...
maxAlpha = 0.1;
    ...
    if ~isnumeric(alpha)
        error('econ:kpsstest:AlphaNonNumeric',...
              'Significance level must be numeric.')
    elseif ~isvector(alpha)
        error('econ:kpsstest:AlphaIsArray',...
              'Significance level must be a scalar or a vector.')
...
for i = 1:nargin
    ...
    paramLength.(inputname(i)) = length(varargin{i});
    numTests = max(numTests,paramLength.(inputname(i)));
    
    if ~isscalar(varargin{i})
        rowOutput = rowOutput || (size(varargin{i},1) == 1);
    end    
...
for i = 1:(nargin-1)
    for j = (i+1):nargin        
        if (paramLength.(inputname(i)) > 1) && (paramLength.(inputname(j)) > 1) ...
            && (paramLength.(inputname(i)) ~= paramLength.(inputname(j)))
            error('econ:kpsstest:ParameterSizeMismatch',...
                  'Vectors ''%s'' and ''%s'' must be the same length.',...
...
for i = 1:nargin
    ...
    if paramLength.(inputname(i)) == 1
        varargout{i} = varargin{i}(ones(numTests,1));
    else
        varargout{i} = varargin{i}(:);  % Column output
    end
...
%-------------------------------------------------------------------------
% Compute the statistic:
function [testStat,testPValue] = getStat(i,sigLevels,T,testReg,CVTable,needPValue)
