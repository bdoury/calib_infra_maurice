...
\figscale{slidesITW2015/processdetail.pdf}{$\widehat{S_{UU}}(f)$, $\widehat{S_{RR}}(f)$, $\widehat{S_{UR}}(f)$ denote respectively the spectrum on channel SUT, the spectrum on channel SREF and the cross-spectrum between them. In the absence of noise the ratio between the 2 responses is given by the ratio $\widehat{S_{UU}}(f)/\widehat{S_{RR}}(f)$}{}{0.7}
 ...
 \newpage
\figscale{figures/synoptic.pdf}{The buffer contains the data for a duration in accordance with the DFT analysis. For example for low frequency range the duration could be $200$ seconds. Usually this duration is in accordance with the inverse of the bandwidth of the filter. In the figure it is assumed that the buffer duration is $200$ seconds and the number of DFTs with an overlap of $50\%$ is $9$. In this case the DFTs can be performed each time we progress in the buffer of $100$ seconds. After 5 times the DFT duration, we average to perform the SCPs corresponding to a duration of $1000$ seconds. If the frequency sampling is $F_{s}=20$ Hz, the number of samples for performing the DFT is $N=1000\times 20/5=4000$. It is worth to notice that we can restrict the calculation to the frequency bins of interest in the band of the filter.}{}{1}

Let us recall the DFT expression, associated to the frequency $kF_s/N$, performing on $N$ samples denoted $x(0),\ldots,x(N-1)$:
\begin{eqnarray}
