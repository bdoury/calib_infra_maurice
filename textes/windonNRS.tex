...
\figscale{bigschema.pdf}{$S_{x}(f)$ spectral matrix de dimension $(M+1)$, $S_{y}(f)$ spectral matrix de dimension $2$, $S_{z}(f)$ spectral matrix de dimension $2$}{fig:bigschema}{0.5}
...
Consider at first that $U(f)=1/M$, $S_M(f)=M$ and $s_{M}(F)=0$ we obtain
\begin{eqnarray*}
\label{eq:Syentries}
