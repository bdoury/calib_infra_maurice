...
\begin{frame}
 ...
 \begin{itemize}
 \item
 closed form expression for the asymptotic probability distributions of the estimators;
 \item
 sizing the statistic of test for the magnitude square coherence (MSC) level;
 \item
...
the MSC;
 ...
 \item
{\color{red} Evaluation on a measurement campaign at station IS26 during several months.}
 \end{itemize}
...
\hspace{-1cm}
\begin{minipage}{7cm}
\figsstit{fulltestbedIS26.pdf}{0.6}
\end{minipage}
&
%\hspace{-0.5cm}
\begin{minipage}{3cm}
...
_{\color{red}\text{spectrum ratio}}
    \times G_{\mathrm{ref}}
\end{eqnarray*}
\end{minipage}
\end{tabular}
...
\item
to circumvent this issue, stationary and ``almost zero-noise'' time segments are selected. Fortunately it exists a test for that based on the coherence.
\item
theoretical results show that to get a $\pm 5\%$ on the gain, the MSC value must be at least $0.96$. 
\item
but because we only have  an estimate $\hMSC$, we have to thresholding at about $0.98$. %for a confidence level of $90\%$ 
\end{itemize}
...
\begin{itemize}
\item
In relationship with the resolution but also the lack of stationarity, we have considered a sequence of durations in decreasing order with the frequencies and do pre-filtering around these frequencies. However we have noticed that the design is not critical.

\item
In the presented results the following filter bank, derived from empirical approach has been used. 
...
The ratio $\dfrac{v_{\mathrm{wind}}}{f}$ can be viewed as a wavelength [Alcoverro, al., JASA,
2002]. Therefore
...
bandwidth $[0.02 - 4]$ Hz;
 \item
$\pm 5\%$ on the response magnitude
\item
...
\begin{itemize}
\item
The experimental results obtained fully validate the calibration method;
\item
