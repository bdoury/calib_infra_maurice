...
\begin{itemize}
\item
Stationarity $+$ (MSC close to 1) solves the problem of underdetermination
\item
Typically, if we want $\pm 5\%$ on the gain, theoretical results show that we need an MSC value 0f $0.96$, but unfortunately we only have  an estimate of the true MSC. Therefore to get a confidence level of $90\%$ we have to thresholding at $0.98$.
\end{itemize}
\figsstit{processdetail.pdf}{0.8}
...
For stationary signals the accuracy on the estimation does depend on the number of time windows we have for averaging the Fourier components. This number appears as the product of the full time duration by the spectral resolution. Hence the idea to use a filter bank with a logarithmic pavement.
