...
\vspace{-24pt}
$$\widehat{G_{\sut}}(f)\approx \dfrac{\widehat{S_{UU}}(f)}{\widehat{S_{UR}}(f)}\times G_{\sref}(f)$$

\begin{itemize}
\item
objective: calibrate the system under test (SUT), i.e. NRS+sensor+digitizer;
\item
two kinds of signals: acoustic and non acoustic (typically wind) with different ranges of velocity;
\item
...
{ the wind appears as spatially coherent for all SUT inlets and SREF inlet. Therefore everything occurs as there is NO noise, and the MSC is almost 1},
\item
...
{a small part of the wind appears as  spatially coherent for near inlets. Therefore a dip artefact is observed}.
\end{itemize}
\end{minipage}
&
